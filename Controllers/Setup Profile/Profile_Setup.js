...
const ProfileRouter = async (req, res, next) => {
    ...
    if (checkUser == null || checkUser.org_registered == false) {
        ...
        try {
            ...
            const profile = await userModel.findOneAndUpdate(
                { _id: "660f4a25c4b6444b2ddd09f0"}, // replace with the organization ID
                { $set: { org_registered: true, org_id: user_id } }, // use $set operator to update the field
                { new: true }, // return the updated document
            );
            if(!profile){
                console.log("sahilllll");
            }
            
            console.log("sahilllll1");
            await profile.save();
            console.log(profile)
